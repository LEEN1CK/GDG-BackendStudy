# ERD(Entity-Relationship Diagram)
    ...
    외래 키(FK): 다른 테이블의 PK를 참조하는 컬럼

# ERD 관계
    다대일(N:1): N개의 컬럼이 1개의 컬럼을 가짐
    일대다(1:N): 1개의 컬럼이 N개의 컬럼을 가짐
    일대일(1:1): 1개의 컬럼이 1개의 컬럼을 가짐
    다대다(N:M): N개의 컬럼이 M개의 컬럼을 가짐

    다대다 구현 시 중간 테이블(연결 엔티티)를 추가
